...
async fn main() -> Result<()> {
    ...
    if args.len() != 3 {
        ...
        eprintln!("This client sends data continuously to test timing");
        std::process::exit(1);
    }
    ...
    // Send data continuously with timing measurements
    println!("Sending data chunks with timing measurements...");
    
    for i in 0..50 {
        let data = format!("Data chunk {} - testing timing isolation\n", i);
        ...
        println!("Chunk {}: Write took {:?}", i, write_time);
        
        // Small delay between chunks
        tokio::time::sleep(Duration::from_millis(100)).await;
        
        // Highlight when we expect slow processing (every 10th chunk)
        if i % 10 == 9 {
            println!("  ↑ Next chunk will be processed slowly by container");
        }
    }
    
    println!("Done sending data. Check timing patterns above.");
    println!("- Coupled version: Write times should vary based on container speed");
    println!("- Decoupled version: Write times should be consistent");
